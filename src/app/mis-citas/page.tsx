...
import { FaCalendarAlt, FaClock, FaComment, FaPaperPlane, FaTrash, FaSpinner } from "react-icons/fa";
...
export default function MisCitasPage() {
  ...
  const styles = {
    ...
    formTitle: { fontSize: '1.2rem', fontWeight: '600', marginBottom: '1rem' },
    ...
    input: { width: '100%', padding: '0.6rem', borderRadius: '12px', border: '1px solid #cbd5e1', fontSize: '0.9rem' },
    textarea: { width: '100%', padding: '0.6rem', borderRadius: '12px', border: '1px solid #cbd5e1', fontSize: '0.9rem', resize: 'vertical' as const, minHeight: '80px' },
    ...
    estadoBadge: (estado: string) => {
      const colors: Record<string, { text: string; color: string; bg: string }> = {
        CONFIRMADA: { text: "Confirmada", color: "#10b981", bg: "#d1fae5" },
        CANCELADA: { text: "Cancelada", color: "#ef4444", bg: "#fee2e2" },
        COMPLETADA: { text: "Completada", color: "#8b5cf6", bg: "#ede9fe" },
        PENDIENTE: { text: "Pendiente", color: "#f59e0b", bg: "#fef3c7" }
      };
      const info = colors[estado] || colors.PENDIENTE;
        ...
        <form onSubmit={handleSubmit}>
          <div style={styles.formRow}>
            <div style={styles.formGroup}>
              ...
              <input type="date" style={styles.input} value={fecha} onChange={(e) => setFecha(e.target.value)} required min={new Date().toISOString().split('T')[0]} />
            </div>
            <div style={styles.formGroup}>
          ...
          <div style={styles.formGroup}>
            ...
            <textarea style={styles.textarea} value={motivo} onChange={(e) => setMotivo(e.target.value)} placeholder="Describe el motivo de tu cita..." />
          </div>
          <button type="submit" style={styles.button} disabled={solicitando}>
            {solicitando ? <FaSpinner style={{ animation: 'spin 1s linear infinite' }} /> : <FaPaperPlane />} Solicitar Cita
          </button>
        </form>
      </div>

      <h2 style={{ fontSize: '1.2rem', fontWeight: '600', marginBottom: '1rem' }}>Mis citas programadas</h2>
            ...
            <div key={cita.id} style={styles.citaCard}>
              <div style={styles.citaInfo}>
                <div style={styles.citaFecha}>
                  ...
                  <span style={styles.estadoBadge(cita.estado)}>
